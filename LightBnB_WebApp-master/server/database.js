...
const getAllProperties = function(options, limit = 10) {
    ...
    // 3
    if (options.owner_id) {
      // console.log("queryParams.length option cyty: ", queryParams.length)
      queryParams.push(`%${options.owner_id}%`);
      // console.log("queryParams.length option cyty inside: ", queryParams.length)
      ...
      if (options.minimum_price_per_night) {
        if (queryParams.length < 1) {
          queryParams.push(100 *parseInt(options.minimum_price_per_night));
      ...
      if (options.maximum_price_per_night) {
        if (queryParams.length < 1) {
          queryParams.push(100 *parseInt(options.maximum_price_per_night));
          queryString += `WHERE cost_per_night < $${queryParams.length} `;
        } else {
          queryParams.push(100 *parseInt(options.maximum_price_per_night));
          queryString += `AND cost_per_night < $${queryParams.length} `;
...
RETURNING *;
`, [
  property.owner_id, property.title, property.description, property.thumbnail_photo_url, property.cover_photo_url, property.cost_per_night, property.parking_spaces,
